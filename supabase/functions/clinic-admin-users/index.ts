...
function failure(request: Request, error: unknown): Response {
  ...
  const map: Record<string, [number, string]> = {
    ...
    INVALID_USERNAME: [
      400,
      "Username must be 3–40 lowercase letters, numbers, dots, underscores, or hyphens.",
    ],
    ...
    ADMIN_MEMBERSHIP_PROTECTED: [
      409,
      "Administrator membership changes are not available here.",
    ],
  ...
  return errorResponse(
    request,
    status,
    code in map ? code : "INTERNAL_ERROR",
    message,
  );
}
...
Deno.serve(async (request) => {
  ...
  if (request.method !== "POST")
    return errorResponse(request, 405, "METHOD_NOT_ALLOWED", "Use POST.");
  ...
  try {
    ...
    if (action === "create")
      return await createUser(request, adminClient, input);
    if (action === "edit" || action === "update" || action === "set_active")
      return await editUser(request, adminClient, user.id, input);
    if (action === "reset_password")
      return await resetPassword(request, adminClient, input);
