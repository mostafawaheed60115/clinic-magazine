...
The `clinic_events_contact` migration added the original event calendar and registration tables. The later `event_request_survey_and_telesales` migration adds configurable telesales WhatsApp and complaints numbers, open/closed survey rounds, and pharmacy event requests. Admins can open one survey at a time and review all submitted pharmacy details. Active members can read the open survey and create requests only while it is open; each member can read only their own submitted rows. Admins can read all requests and manage survey state. Brand IDs are checked against the catalog and their Arabic/English names are snapshotted for historical display. RLS is enabled and forced on both new tables.
