...
export function authPage(mode = "unconfigured", admin = false) {
  ...
  if (admin) {
    return `<section class="${a.adminAuthShell}" aria-labelledby="login-title"><div class="${a.adminAuthCard}"><header class="${a.adminAuthHeader}"><div class="${a.adminAuthBrand}"><img src="/assets/clinic-logo-transparent.png" alt="Clinic" width="87" height="87" /><div><span>${t("adminLoginBadge")}</span><strong>Clinic</strong></div></div>${localeButton}</header><div class="${a.adminAuthDivider}" aria-hidden="true"></div><div class="${a.adminAuthBody}"><p class="${a.adminAuthEyebrow}">${t("adminLoginAccess")}</p><h1 id="login-title" tabindex="-1">${t("adminLoginTitle")}</h1><p class="${a.adminAuthText}">${t("adminLoginText")}</p>${authForm(mode, a.adminAuthForm, a.adminAuthSubmit, a.adminLoginError)}<div class="${a.adminAuthFootnote}"><span>${t("adminLoginFootnote")}</span>${novaAttribution({ withLogo: true })}</div></div></div></section>`;
  }
  return `<section class="${m.loginShell}" aria-labelledby="login-title"><div class="${m.loginTopbar}">${localeButton}</div><div class="${m.loginPanel}"><div class="${m.loginPanelInner}"><img class="${m.loginPanelLogo}" src="/assets/clinic-logo-transparent.png" alt="Clinic" width="155" height="155" /><h1 id="login-title" tabindex="-1">${t("loginTitle")}</h1><p class="${m.loginText}">${t("loginText")}</p>${authForm(mode, m.loginForm, m.loginSubmit, m.loginError)}<div class="${m.loginFooter}"><span>${t("loginFootnote")}</span>${novaAttribution({ withLogo: true })}</div></div></div></section>`;
}
