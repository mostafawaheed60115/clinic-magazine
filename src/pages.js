...
export function authPage(mode = "unconfigured") {
  ...
  return `<section class="${m.loginShell}" aria-labelledby="login-title"><div class="${m.loginTopbar}"><span>${t("loginSecure")}</span><button type="button" id="login-locale-toggle" class="${s.ghost}">${icon("globe")}<span>${locale === "ar" ? "English" : "العربية"}</span></button></div><div class="${m.loginVisual}"><a class="${m.loginLogo}" href="#/offers" aria-label="Clinic"><img src="/assets/new_logo.png" alt="Clinic" width="96" height="96" /></a><div class="${m.loginCaption}"><span>${t("loginEyebrow")}</span><p>${t("loginTitle")}</p></div><div class="${m.loginVisualMeta}"><span>${t("loginVisualMeta")}</span><span>${t("loginLocation")}</span></div></div><div class="${m.loginPanel}"><div class="${m.loginPanelInner}"><div class="${m.loginBrand}"><img src="/assets/new_logo.png" alt="" width="46" height="46" /><span><strong>Clinic</strong><small>${t("loginEyebrow")}</small></span></div><h1 id="login-title" tabindex="-1">${t("loginTitle")}</h1><p class="${m.loginText}">${t("loginText")}</p>${demo ? `<p class="${m.demoLoginNote}">${t("demoWarning")}<br /><small>${t("demoCredentials")}</small></p>` : mode === "unconfigured" ? `<p class="${m.demoLoginNote}">${t("configuredLoginText")}</p>` : ""}<form id="login-form" class="${m.loginForm}" novalidate><div class="${m.loginField}"><label for="login-user">${t("username")}</label><input id="login-user" name="user" autocomplete="username" autocapitalize="none" spellcheck="false" dir="ltr" required aria-describedby="login-user-error" /></div><span id="login-user-error" class="${s.error}"></span><div class="${m.loginField}"><div class="${m.loginLabelRow}"><label for="login-pass">${t("password")}</label><span>${t("loginPasswordHint")}</span></div><div class="${m.passwordWrap}"><input id="login-pass" name="pass" type="password" autocomplete="current-password" dir="ltr" required aria-describedby="login-pass-error login-error" /><button type="button" id="login-password-toggle" class="${s.iconButton}" aria-label="${t("showPassword")}" aria-pressed="false">${icon("eye")}</button></div><span id="login-pass-error" class="${s.error}"></span></div><p id="login-error" role="alert" class="${s.error}"></p><button class="${s.button} ${s.primary} ${m.loginSubmit}" type="submit"><span>${t("signIn")}</span>${arrow()}</button></form><div class="${m.loginFooter}"><span>${t("loginFootnote")}</span><span class="${m.loginSecureMark}">${icon("check")}${t("loginSecureMark")}</span></div></div></div></section>`;
}
