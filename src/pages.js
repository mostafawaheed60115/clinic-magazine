...
export function authPage(mode = "unconfigured", admin = false) {
  ...
  return `<section class="${m.loginShell}" aria-labelledby="login-title"><div class="${m.loginTopbar}">${localeButton}</div><div class="${m.loginPanel}"><div class="${m.loginPanelInner}"><img class="${m.loginPanelLogo}" src="/assets/clinic-logo-transparent.png" alt="Clinic" width="130" height="130" /><h1 id="login-title" tabindex="-1">${t("loginTitle")}</h1><p class="${m.loginText}">${t("loginText")}</p>${authForm(mode, m.loginForm, m.loginSubmit, m.loginError)}<div class="${m.loginFooter}"><span>${t("loginFootnote")}</span>${novaAttribution({ withLogo: true })}</div></div></div></section>`;
}
