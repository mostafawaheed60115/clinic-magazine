...
function offerCard(offer, data) {
  ...
  return `<div class="${m.coverCopy}"><span class="${m.brandName}" dir="ltr">${esc(brand?.name_en || "Clinic")}</span><h2>${esc(nameOf(offer))}</h2><p>${esc(offer[`description_${locale}`] || offer.description_en || "")}</p>${link(`#/brand/${encodeURIComponent(offer.company_id)}`, t("discover"))}</div><div class="${m.coverVisual}">${image(offer.img_link, offer[`description_${locale}`] || t("sampleArt"), "", "eager")}<span class="${m.sampleTag}">${t("sample")}</span></div>`;
}
export function authPage(mode = "unconfigured") {
  const demo = mode === "demo";
  return `<section class="${m.loginShell}"><div class="${m.loginVisual}"><a class="${m.loginLogo}" href="#/offers" aria-label="Clinic"><img src="./assets/clinic-symbol.svg" alt="" width="58" height="44" /><span>clinic</span></a><div class="${m.loginCaption}"><span>${t("loginEyebrow")}</span><p>${t("loginTitle")}</p></div></div><div class="${m.loginPanel}"><div class="${m.loginPanelInner}"><span class="${m.eyebrow}">${t("loginEyebrow")}</span><h1 tabindex="-1">${t("loginTitle")}</h1><p class="${m.loginText}">${t("loginText")}</p>${demo ? `<p class="${m.demoLoginNote}">${t("demoWarning")}<br /><small>${t("demoCredentials")}</small></p>` : mode === "unconfigured" ? `<p class="${m.demoLoginNote}">${t("configuredLoginText")}</p>` : ""}<form id="login-form" class="${m.loginForm}" novalidate><div class="${m.loginField}"><label for="login-user">${t("username")}</label><input id="login-user" name="user" autocomplete="username" dir="ltr" required aria-describedby="login-user-error" /><span id="login-user-error" class="${s.error}"></span></div><div class="${m.loginField}"><label for="login-pass">${t("password")}</label><div class="${m.passwordWrap}"><input id="login-pass" name="pass" type="password" autocomplete="current-password" dir="ltr" required aria-describedby="login-pass-error login-error" /><button type="button" id="login-password-toggle" class="${s.iconButton}" aria-label="${t("showPassword")}" aria-pressed="false">${icon("eye")}</button></div><span id="login-pass-error" class="${s.error}"></span></div><p id="login-error" role="alert" class="${s.error}"></p><button class="${s.button} ${s.primary} ${m.loginSubmit}" type="submit"><span>${t("signIn")}</span>${arrow()}</button></form><div class="${m.loginLanguage}"><button type="button" id="login-locale-toggle" class="${s.ghost}">${icon("globe")}<span>${locale === "ar" ? "English" : "العربية"}</span></button></div></div></div></section>`;
}
export function offersPage(data) {
  return `<div class="${m.intro}"><div><h1 tabindex="-1">${t("magazine")}</h1><p>${t("subtitle")}</p></div><span class="${m.edition}">${icon("spark")}${t("demoCatalog")}</span></div>${data.offers.length ? `<section aria-roledescription="${locale === "ar" ? "عارض عروض" : "carousel"}" aria-label="${t("offers")}" id="carousel"><div class="${m.cover}" id="cover" tabindex="0">${offerCard(data.offers[0], data)}</div><div class="${m.carouselFoot}"><div class="${m.slideMeta}" id="slide-position" aria-live="polite"><strong>${number(1)} / ${number(data.offers.length)}</strong><span>${t("offers")}</span></div><div class="${m.dots}">${data.offers.map((offer, i) => `<button class="${m.dot}" data-slide="${i}" aria-label="${t("slide")} ${number(i + 1)}: ${esc(nameOf(offer))}" aria-current="${i === 0}"></button>`).join("")}</div><div class="${m.carouselArrows}"><button class="${s.iconButton}" id="previous-offer" aria-label="${t("previous")}" disabled>${icon("arrow", s.flip)}</button><button class="${s.iconButton}" id="next-offer" aria-label="${t("next")}" disabled>${icon("arrow")}</button></div></div></section>` : empty("offersEmpty", "emptyText")}<section class="${m.section}"><div class="${m.sectionHead}"><div><h2>${t("curated")}</h2><p>${t("curatedText")}</p></div><a class="${m.textLink}" href="#/brands">${t("allBrands")}${arrow()}</a></div><div class="${m.brandGrid}">${data.companies.slice(0, 5).map(brandTile).join("")}</div>${!data.companies.length ? empty() : ""}</section>`;
}
