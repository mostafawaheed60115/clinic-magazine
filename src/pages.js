...
function brandTile(brand) {
  return `<a href="#/brand/${encodeURIComponent(brand.id)}" class="${m.brandTile}" aria-label="${esc(nameOf(brand))}"><span class="${m.brandIdentity}" dir="ltr">${brand.logo_url ? image(brand.logo_url, "", m.brandLogo) : ""}<strong dir="auto">${esc(nameOf(brand))}</strong></span></a>`;
}
...
export function brandPage(data, route) {
  ...
  return `<section class="${m.brandShowcase}"><nav class="${m.breadcrumbs}" aria-label="${t("back")}"><a href="#/brands">${t("brands")}</a>${arrow()}<span aria-current="page">${esc(nameOf(brand))}</span></nav><div class="${m.brandHero}"><div class="${m.brandBadge}">${wordmark(brand)}</div><div class="${m.brandHeroContent}"><h1 tabindex="-1">${esc(nameOf(brand))}</h1><p>${t("collection")}${brand.parent_id ? ` · ${esc(nameOf(data.companies.find((b) => b.id === brand.parent_id) || {}))}` : ""}</p>${brand.phone ? `<a href="tel:${esc(brand.phone.replace(/[^+\d]/g, ""))}"><bdi>${esc(brand.phone)}</bdi></a>` : ""}</div></div><div class="${m.toolbar}">${searchBox(q, "searchProducts")}<span class="${m.count}" role="status">${number(list.total)} ${t("results")}</span></div><div class="${m.products}">${list.items.map(productCard).join("")}</div>${list.total ? "" : empty(q ? "noResults" : "empty", q ? "noResultsText" : "emptyText", m.brandProductEmpty)}${pagination(list.page, list.pages)}<p class="${m.note}">${t("stockNote")}</p></section>`;
}
