...
export function novaAttribution({ withLogo = false } = {}) {
  return `<div class="${m.novaAttribution}" dir="ltr">${withLogo ? `<img class="${m.novaLogo}" src="/assets/Remove_background_of_logo_202608080142.jpeg" alt="Nova Solutions" width="28" height="28" />` : ""}<div><a href="${NOVA_URL}" target="_blank" rel="noopener noreferrer">${t("novaCredit")}</a><a class="${m.novaPhone}" href="tel:+2015099999283">${t("novaPhone")}</a></div></div>`;
}
...
export function authPage(mode = "unconfigured", admin = false) {
  ...
  return `<section class="${m.loginShell}" aria-labelledby="login-title"><div class="${m.loginTopbar}">${localeButton}</div><div class="${m.loginPanel}"><div class="${m.loginPanelInner}"><img class="${m.loginPanelLogo}" src="/assets/clinic-logo-transparent.png" alt="Clinic" width="96" height="96" /><h1 id="login-title" tabindex="-1">${t("loginTitle")}</h1><p class="${m.loginText}">${t("loginText")}</p>${authForm(mode, m.loginForm, m.loginSubmit, m.loginError)}<div class="${m.loginFooter}"><span>${t("loginFootnote")}</span>${novaAttribution({ withLogo: true })}</div></div></div></section>`;
}
